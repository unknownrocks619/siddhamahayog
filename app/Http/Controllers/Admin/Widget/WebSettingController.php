...
class WebSettingController extends Controller
{
    ...
    public function Update(Request $request)
    {
        ...
        $settings->where('name', "website_url")->first()->value = ($request->website_url) ? $request->website_url : ((env("APP_DEBUG")) ? "localhost:8000" : "https://siddhamahayog.org");
        ...
        $settings->where('name', "application")->first()->value = ($request->application) ? $request->application : $settings->where('name', "application")->first()->value;
        $settings->where('name', "account_verification")->first()->value = ($request->account_verification) ? $request->account_verification : $settings->where('name', "account_verification")->first()->value;
