...
class ZoomAPIController extends Controller
{
    ...
    public function store_zoom_meeting(ZoomAPIRequest $request) {
        ...
        if ($request->program) {
            ...
            if (! $zoom_meeting->where('meeting_id',$zoom_meeting->id)->where('program_id',$request->program)->exists()) {
                $zoom_program->meeting_id = $zoom_meeting->id;
                $zoom_program->program_id = $request->program;
                $zoom_program->is_live = false;
                $zoom_program->save();
            }
        }
        return response(["success"=>true,"message"=>"New Meeting Created."],200);
    }
