@php
        $leader = \App\Models\UserFamilyGroup::where('leader_id',\Cache::store('file')->get('u_l')->user_detail_id)
                            ->where('approved',true)
							->with(["member_detail"])
...
@if ($leader->count())
    <form method="post" id="zoom_attendancce" action="{{ route('public.event.public_make_group_attendance',encrypt($record->event_id)) }}">
    @csrf
    <div class='modal-header'>
        <h5 class='modal-title'>Attendance for `<span class='text-danger'>{{ $record->event_source->sibir_title }}</span>`</h5>
    </div>
...
@else
	...
	<div class='modal-footer'>
		<a href='#' class='btn btn-secondary disabled action'>Join Session</a>
	</div>
    <script>
        ...
        $(document).ready(function(){
            $.ajax({
                ...
                url : "{{ route('public.event.public_make_single_attendance',encrypt($record->event_id)) }}",
                headers : {
