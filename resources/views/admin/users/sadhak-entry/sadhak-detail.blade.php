...
<section id="headers">
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <p>Donation Received : {{ $user_detail->donation->amount ?? 0 }}</p>
                </div>
    ...
    <div class="row" id='profile_detail_card'>
                ...
                <div class="card-body">
                    ...
                    <div class='row' id='donation'>
                        <div class="col-md-12">
                            <a data-toggle="modal" 
                                data-target="#display_modal"
                                 href='{{ route("modals.display",["modal"=>"donation_modal","user_detail_id"=>$user_detail->id]) }}' class='btn btn-primary btn-block'>Add Donation</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class='col-md-6'>
            <div class='card'>
                @empty($user_detail->user_health_data)
                @else
                <div class='card-header'>
                    <h3 class='card-title'>Health Condition</h3>
                </div>
                @endempty
                <div class='card-body'>
                    @forelse ($user_detail->user_health_data as $health_data )
                        <div class='border-left border-primary @if($health_data->have_physical_difficulties || $health_data->have_mental_problem) border-danger @endif pl-2 mb-2'>
                            <p class='card pb-0 mb-0 @if($health_data->have_physical_difficulties) border border-primary px-2 mb-1 @endif'>General Health Problem : 
                                <strong>{{ ($health_data->have_physical_difficulties) ? "Yes" : "No" }}</strong>
                                @if($health_data->have_physical_difficulties)
                                <br />
                                <label class='label-control'>Problem / Medication Status</label>
                                <textarea class='form-control' disabled>{{ $health_data->describe_physical_difficulties }}</textarea>
                                @endif
                            </p>
                            <p class='pb-0 mb-0 card @if($health_data->have_mental_problem) border border-primary px-2 @endif'>Mental Health Problem: 
                                <strong>{{ ($health_data->have_mental_problem) ? "Yes" : "No" }}</strong>
                                @if($health_data->have_mental_problem)
                                <br />
                                <label class='label-control'>Problem / Medication Status</label>
                                <textarea class='form-control' disabled>{{ $health_data->describe_mental_difficulties }}</textarea>
                                @endif
                            </p>
                            
                            <label class='label-control text-info pt-0 mt-0'>Information Stored On : {{ date("D d,M Y",strtotime($health_data->created_at)) }}</label>

                        </div>
                    @empty
                    <h5 class='text-danger'>
                        Health Information is missing.
                    </h5>
                    @endforelse
                </div>
            </div>
        ...
        <div class='col-md-6'>
            <div class='card'>
                <div class='card-header'>
                    @empty($user_detail->sibir_records->count())
                        <h4 class='card-title text-danger'>No Past Sibir Record</h4>
                    @else
                        <h4 class='card-title'>Sadhana Attend History</h4>
                    @endempty
                </div>
                <div class='card-body'>
                @forelse ($user_detail->sibir_records as  $sibir_history )
        ...
        <div class='col-md-6'>
            <div class='card'>
                <div class='card-header'>
                    <h4 class='card-title'>Sadhak Qulification Status</h4>
                </div>
                <div class='card-body'>
                    <p>
                        Pre Video Session Status : <span class='text-success'>Completed</span>
                        <br /><span class='text-info'> Ongoing [ 101 Minute Left ]</span> 
                        <br /><span class='text-warning'>Pending</span>
                    </p>
                    <p>
                        Sadhana Attendance : <span>Not Qualified</span>
                        <br />
                        Qualified (Not Attended Yet)
                    </p>
                    <p>Post Sadhana Study : <span class='text-info'>Onoing [190 Minute Remaining]</span></p>
                </div>
            </div>
        </div>
