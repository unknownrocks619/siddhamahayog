@php
    $program = \App\Models\Program::where('id',request()->get('program'))->with('students')->first();
    $programSection = \App\Models\ProgramSection::where('id',request()->get('section'))->first();
@endphp
<form method="post" class="ajax-form" action="{{ route('admin.program.live_program.merge.store',['program'=>$program]) }}">
    ...
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label for="program_name">Section <sup class="text-danger">*</sup></label>
                    <span class="form-control bg-light">{{$programSection->section_name}}</span>
                </div>
            </div>
        </div>
        <div class="row my-4">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <input data-action="{{route('admin.program.sections.admin_list_all_section',['program' => request()->program,'section' => $programSection->getKey()])}}"
                           type="text"
