@php
    ...
    $batch = \App\Models\Batch::where('id',request()->get('batch'))->first();
@endphp
<form method="post" class="ajax-form" action="{{ route('admin.program.live_program.merge.store',['program'=>$program]) }}">
    ...
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label for="program_name">Batch <sup class="text-danger">*</sup></label>
                    <span class="form-control bg-light">{{$batch->batch_name}} ({{$batch->batch_year}} - {{$batch->batch_month}})</span>
                </div>
            </div>
        </div>
        <div class="row my-4">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    <input data-action="{{route('admin.program.batches.admin_batch_list',['program' => request()->program,'batch' => $batch->getKey()])}}"
                           type="text"
