...
@if ( $row->liveProgram->count())
    ...
    @foreach ($row->liveProgram as $live_program)
        @if($live_program->merge)
            ...
            @endforeach
        @endif
        <button class="mb-1 btn btn-danger btn-sm data-confirm" data-title="Confirm Your Action" data-confirm="You are about to end the session. The user will no longer be able to join. This will not affect the Zoom session." data-method="post" data-action="{{route('admin.program.live_program.end', [$live_program->id])}}">
  ...
  @endforeach
    <br />
    <button data-action='{{route('admin.modal.display',['view' => 'live.merge-selection','program' => $row->getKey(),'live' => $live_program->getKey()])}}{{--route('admin.program.live_program.merge.view', [$row->id, $live_program->id])--}}'
            data-bs-toggle='modal' data-bs-target='#mergeSession'  class='btn btn-info btn-sm mb-1 ajax-modal'>
        Merge
    </button>
@endif
