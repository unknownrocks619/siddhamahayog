                ...
                <div class=" d-flex align-items-center flex-column">
                    @if ($member->profileImage)
                        <img class="img-fluid rounded mb-3 pt-1 mt-4"
                            src="{{ \App\Classes\Helpers\Image::getImageAsSize($member->profileImage->filepath, 's') }}"
            ...
            <div class="info-container">
                ...
                @if (adminUser()->role()->isSuperAdmin())
                    <div class="d-flex justify-content-center">
                        <a href="#" class="me-3 data-confirm btn btn-label-danger suspend-user"
                            data-confirm="You will be Logged In As `{{ $member->full_name }}`" data-method="post"
                            data-action="{{ route('admin.members.admin_login_as_user', $member->getKey()) }}">View
                            As</a>
                    </div>
                @endif
            </div>
    ...
    @if (adminUser()->role()->isSuperAdmin())
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <form class="ajax-form" action="{{ route('admin.members.admin_change_user_password', $member->id) }}"
                    onsubmit="return confirm('You are about to change the password for current user. Are you sure? this action cannot be undone')"
                    method="put">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input required type="password" name="password" id="user_new_passowrd"
                                    class="form-control" />
                                @error('password')
